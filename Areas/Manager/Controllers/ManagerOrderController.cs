...
namespace Bloomie.Areas.Manager.Controllers
{
    ...
    public class ManagerOrderController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? statusFilter, string? paymentStatusFilter, DateTime? fromDate, DateTime? toDate)
        {
        ...
        public async Task<IActionResult> ConfirmOrder(int id)
        {
            ...
            // �🚀 TỰ ĐỘNG PHÂN CÔNG SHIPPER (Round Robin)
            ...
            if (assignmentSuccess)
            {
                ...
                if (assignedOrder?.ShipperId != null)
                {
                    ...
                    TempData["success"] = $"Đã xác nhận đơn hàng và phân công cho shipper: {shipper?.FullName ?? "N/A"}. Shipper có 3 phút để xác nhận nhận đơn hàng.";
                }
                else
                {
                    TempData["success"] = "Đã xác nhận đơn hàng và phân công shipper thành công.";
                }
            }
            else
            {
