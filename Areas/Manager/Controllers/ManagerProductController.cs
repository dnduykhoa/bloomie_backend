...
namespace Bloomie.Areas.Manager.Controllers
{
    ...
    public class ManagerProductController : Controller
    {
        ...
        public ManagerProductController(ApplicationDbContext context, IWebHostEnvironment env)
        {
            _context = context;
            _env = env;
        }
        ...
        {
            // Simple placeholder - in production, use a library like EPPlus or ClosedXML
            var query = _context.Products
                .Include(p => p.ProductCategories).ThenInclude(pc => pc.Category)
                .AsQueryable();
            ...
            // Return CSV format for simplicity
            var csv = new System.Text.StringBuilder();
            csv.AppendLine("STT,Tên sản phẩm,Giá,Số lượng,Kiểu,Danh mục");

            int stt = 1;
            foreach (var p in products)
            {
                var shape = p.ProductCategories?.FirstOrDefault(pc => pc.Category.Type == (int)CategoryType.Shape)?.Category?.Name ?? "";
                var categories = string.Join(", ", p.ProductCategories?.Where(pc => pc.Category.Type != (int)CategoryType.Shape).Select(pc => pc.Category.Name) ?? new List<string>());
                csv.AppendLine($"{stt},{p.Name},{p.Price},{p.StockQuantity},{shape},{categories}");
                stt++;
            }
